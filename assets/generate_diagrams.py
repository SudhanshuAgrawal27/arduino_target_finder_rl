...
def make_system_diagram(out_file="assets/system-diagram.png"):
    fig, ax = plt.subplots(figsize=(11, 3.9), dpi=160)
    ...
    _card(ax, (0.15, 0.15), 10.7, 3.6)

    host_xy, host_w, host_h = (0.7, 0.55), 4.1, 2.7
    ard_xy, ard_w, ard_h = (6.2, 0.55), 4.1, 2.7

    _box(ax, host_xy, host_w, host_h, "HOST MACHINE (python)", [
        "GridEnvironment · simulator.py",
        "ActorCritic policy · network.py",
        "Serial client · led_board_client.py",
    ])
    _box(ax, ard_xy, ard_w, ard_h, "ARDUINO", [
        "LED matrix · WS2812B / MAX7219",
        "Photoresistor (LDR)",
        "Serial listener · led_serial_listener.ino",
    ])

    mid_y_top = host_xy[1] + host_h * 0.62
    mid_y_bot = host_xy[1] + host_h * 0.30
    x0 = host_xy[0] + host_w
    x1 = ard_xy[0]

    ax.annotate("", xy=(x1, mid_y_top), xytext=(x0, mid_y_top),
                arrowprops=dict(arrowstyle="-|>", color=ACCENT, lw=2))
    ax.text((x0 + x1) / 2, mid_y_top + 0.16, "agent / target position",
    ...
    ax.text((x0 + x1) / 2, mid_y_bot - 0.16, "LDR brightness reading",
